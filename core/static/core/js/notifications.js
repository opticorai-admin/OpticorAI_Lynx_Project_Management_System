...
$(document).ready(function() {
    // Dropdown: mark as read when a notification item is clicked
    // Keeps navigation behavior intact, updates badges optimistically, and sends a background GET
    $(document).on('click', '.mark-read-link', function() {
        var id = $(this).data('id');
        if (!id) { return; }

        // Optimistically update UI
        decrementGlobalUnreadBadges();
        // Remove the clicked item from dropdown to reflect read state immediately
        $(this).remove();

        var url = '/notifications/' + id + '/mark-read/';
        try {
            // Use an image beacon so the GET is dispatched even if the browser navigates immediately
            var img = new Image();
            img.src = url + (url.indexOf('?') === -1 ? '?' : '&') + 't=' + Date.now();
        } catch (e) {
            // Fallback: fire-and-forget fetch with keepalive
            try { fetch(url, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' }, keepalive: true }); } catch(_) {}
        }
    });
    ...
    // Decrement global unread badges (header + sidebar) by one
    function decrementGlobalUnreadBadges() {
